package com.jishi.daichao.permission;
...
import com.jishi.daichao.permission.bean.Permissions;
import com.jishi.daichao.permission.bean.Special;
import com.jishi.daichao.permission.callbcak.CheckRequestPermissionListener;
import com.jishi.daichao.permission.callbcak.CheckRequestPermissionsListener;
import com.jishi.daichao.permission.callbcak.CheckStatusCallBack;
...
import com.jishi.daichao.permission.checker.CheckerFactory;
import com.jishi.daichao.permission.debug.PermissionDebug;
import com.jishi.daichao.permission.request.PermissionRequester;
...
public class SoulPermission {
    ...
    /**
     * 检查特殊权限  SYSTEM_ALERT，  NOTIFICATION
     * @param special
     * @param
     */
    public boolean checkSpecial(@NonNull Special special) {
        boolean permissionResult = checkSpecialPermission(special);
        if (permissionResult || Build.VERSION.SDK_INT < KITKAT) {
            return  true;
        }
        return false;
    }
    ...
    private void requestPermissions(final Permissions permissions, final CheckRequestPermissionsListener listener) {
        checkStatusBeforeDoSomething(new CheckStatusCallBack() {
            ...
            public void onStatusError() {
                //没有回调，导致课后网启动页异常
                listener.onAllPermissionOk(permissions.getPermissions());
            }
        });
    }
    ...
    public void requestSpecialPermission(final Special specialPermission, final SpecialPermissionListener listener) {
        checkStatusBeforeDoSomething(new CheckStatusCallBack() {
            @Override
            public void onStatusOk(Activity activity) {
                new PermissionRequester(activity)
            ...
            public void onStatusError() {
                //特殊情况，没有回调，默认授权了
                listener.onGranted(specialPermission);
            }
        });
